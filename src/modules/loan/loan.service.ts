...
import {
  LoanOfferDto,
  OriginatorDto,
  PendingOriginatorDto,
  PendingResponseDto,
  PersonalLoanResponseDto,
} from './dtos/response.loan.dto';
...
export class LoanService extends CrudService<Loan> {
  ...
  async generate(loanData: CreateLoanDto) {
    ...
    try {
      const personalInformation = await this.personaService.create(
        loanData.personalInformation,
      );
      const loanInformation = await this.loanInformationService.create(
        loanData.loanInformation,
      );
      const mortgageInformation = await this.mortgageInformationService.create(
        loanData.mortgageInformation,
      );
      const creditInformation = await this.creditInformationService.create(
        loanData.creditInformation,
      );
      const financialInformation =
        await this.financialInformationService.create(
          loanData.financialInformation,
        );
      const educationInformation =
        await this.educationInformationService.create(
          loanData.educationInformation,
        );
      const legalInformation = await this.legalInformationService.create(
        loanData.legalInformation,
      );
      const clientTags = await this.clientTagsService.create(
        loanData.clientTags,
      );
  ...
  async generateDynamicResponse(
    loanId: string,
  ): Promise<PersonalLoanResponseDto> {
    const generateLoanOffer = (): LoanOfferDto => {
      const originators: OriginatorDto[] = [
        {
          ...
          disclaimer:
            'Suspendisse rhoncus magna erat, nec rhoncus leo elementum eget.',
        },
        {
          key: 'avant',
          name: 'Avant',
      ...
      const originator =
        originators[Math.floor(Math.random() * originators.length)];
      ...
      const maxApr = parseFloat(
        (Math.random() * (35.99 - 5.99) + 5.99).toFixed(2),
      );
      ...
      const maxMonthlyPayment = parseFloat(
        (monthlyPayment + Math.random() * 50).toFixed(2),
      );
      const minMonthlyPayment = parseFloat(
        (monthlyPayment - Math.random() * 50).toFixed(2),
      );
      const meanMonthlyPayment = parseFloat(
        ((maxMonthlyPayment + minMonthlyPayment) / 2).toFixed(2),
      );
    ...
    const pendingResponses: PendingResponseDto[] = [
      {
        partner: {
          ...
          disclaimer:
            'Suspendisse rhoncus magna erat, nec rhoncus leo elementum eget.',
          imageUrl:
            'https://images.evenfinancial.com/old/images/avant/avant_120.png',
        },
        productTypes: ['loan'],
      },
    ];
